...
class Profile(db.Model):
    ...
    on_work = db.Column(db.Boolean, nullable=False, default=False)
...
def index():
    ...
    pagination = db.session.query(Record.id, Record.time_created, Profile.name, Record.id_person,
                                  Record.in_or_out).join(Profile, isouter=True).order_by(
        Record.time_created.desc()).paginate(
        page, per_page=25)
    return render_template('index.html', pagination=pagination)


@app.route('/system-json/get-record', methods=['GET'])
def get_record():
    page = request.args.get('page', 1, type=int)
    pagination = db.session.query(Record.id, Record.time_created, Profile.name, Record.id_person,
                                  Record.in_or_out).join(Profile, isouter=True).paginate(
        page, per_page=25)
    results = {
        "records": [{"id": m.id, "time": m.time_created, "user": m.name} for m in pagination.items],
        "pagination": {
...
@app.route('/system-json/get-profile', methods=['GET'])
def get_profile():
    page = request.args.get('page', 1, type=int)
    pagination = db.session.query(Profile.id, Profile.name, Profile.phone_number, Profile.custom_hashtag,
                                  Profile.email_address, Profile.photo_path, Profile.on_work).paginate(
        page, per_page=25)
    results = {
        "profiles": [{"id": m.id,
                      "name": m.name,
                      "phone_number": m.phone_number,
                      "custom_hashtag": m.custom_hashtag,
                      "email_address": m.email_address,
                      "on_work": m.on_work,
                      "photo_path": m.photo_path} for m in pagination.items],
        "pagination": {
            "count": pagination.total,
            "page": page,
            "per_page": pagination.per_page,
...
@app.route('/system-json/get-card', methods=['GET'])
def get_card():
    page = request.args.get('page', 1, type=int)
    pagination = db.session.query(Card.id, Card.serial_number, Card.card_sak, Card.id_person, Card.card_type,
                                  Card.firmware).paginate(
        page, per_page=25)
    results = {
        "cards": [{"id": m.id,
                   "serial_number": m.serial_number,
                   "card_sak": m.card_sak,
                   "person_id": m.id_person,
                   "card_type": m.card_type,
                   "firmware": m.firmware} for m in pagination.items],
        "pagination": {
            "count": pagination.total,
            "page": page,
            "per_page": pagination.per_page,
    ...
    if bool(request_data):
        if ('name' in request_data) & ('phone_number' in request_data) & ('custom_hashtag' in request_data) \
            ...
            if (request_data['auth_key'] == auth_key):
                ...
                profile1 = Profile(name=name, phone_number=phone_number, custom_hashtag=custom_hashtag,
                                   email_address=email_address, photo_path=photo_path)
    ...
    if bool(request_data):
        if (request_data['auth_key'] == auth_key):
            if ('serial_number' in request_data) & ('user_id' in request_data) \
                ...
                card_type = request_data['card_type']
                firmware = request_data['firmware']
                card = Card(serial_number=serial_number, card_sak=card_sak, id_person=person_id, card_type=card_type,
                            firmware=firmware)
                db.session.add(card)
                db.session.commit()
                return jsonify(info='JSON parsing success', message='Data written in DB')
